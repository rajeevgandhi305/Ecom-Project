...
//         <div className='relative hidden md:flex items-center space-x-4 font-bold text-xl'>
//         <h1 className='text-white bn font-bold text-2xl'>Username</h1>
//           <FaUserCircle className='text-white cursor-pointer' size={32} onClick={handleDropdown} />
        
//           {dropdown && (
//             <div className='absolute top-12 right-0 bg-white rounded-lg shadow-md p-2 z-10'>
...
const Navbar = () => {
  const [nav, setNav] = useState(false);
  const [dropdown, setDropdown] = useState(false);
  const [mobileUsernameClicked, setMobileUsernameClicked] = useState(false);
  ...
  const handleMobileUsernameClick = () => {
    setMobileUsernameClicked(!mobileUsernameClicked);
  };
  ...
  return (
    <div className='relative w-full bg-gray-950'>
      <div className='flex justify-between mx-auto rounded-3xl p-4 shadow-md'>
        ...
        {/* User Icon and Dropdown */}
        <div className='relative hidden md:flex items-center space-x-4 font-bold text-xl'>
          <h1 className='text-white bn font-bold text-2xl cursor-pointer' onClick={handleDropdown}>Username</h1>
          <FaUserCircle className='text-white cursor-pointer' size={32} onClick={handleDropdown} />
      ...
      >
        <ul className='flex flex-col w-full h-full items-start justify-start mt-20 px-8'>
          ...
          <li className='font-bold text-3xl py-4' onClick={handleNav}>
            <h1 className="text-blue-900 hover:text-red-600 cursor-pointer" onClick={handleMobileUsernameClick}>Username</h1>
            {mobileUsernameClicked && (
              <div className='bg-white rounded-lg shadow-md p-2'>
                <Link to="/register" className='block px-4 py-2 text-gray-800 hover:bg-gray-200 ' onClick={() => setMobileUsernameClicked(false)}>Register</Link>
                <Link to="/logout" className='block px-4 py-2 text-gray-800 hover:bg-gray-200' onClick={() => setMobileUsernameClicked(false)}>Logout</Link>
              </div>
            )}
          </li>
        </ul>
